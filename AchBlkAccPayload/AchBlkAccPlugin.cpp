...
class AchBlkAccPlugin: public Plugin
{
public:
	AchBlkAccPlugin( const std::string& name, const std::string& version ): Plugin( name, version ) {}
	bool hasNamespace( const string& aNamespace )
	{
		return ( aNamespace == "urn:swift:xs:BlkAcc" );
	}
	RoutingMessageEvaluator* newInstance( const XERCES_CPP_NAMESPACE_QUALIFIER DOMDocument* document )
	{
		return new AchBlkAccPayload( document );
	}
};
...
#endif
{
	return new AchBlkAccPlugin( "AchBlkAcc", "0.1" );
}
}
